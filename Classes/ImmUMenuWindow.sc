	...
	*makeWindow {
		...
		.tree_(
			OEM(
				...
				'      ', (),
				'check system is okay', {
					var a = ULib.servers[0].loads.sum == 0;
					var b = UGroup.all.collect(_.groups).as(Array).mreduce(Array).isEmpty;
					var c = UBus.gBuses.as(Array).isEmpty;
					var d = RichBuffer.allBuffers.as(Array).isEmpty;
					if(a && b && c && d){
						"System is okay".postln;
					}{
						"System is not okay, fixing it".postln;
						ULib.servers[0].loads = ULib.servers[0].loads.collect{ 0.0 };
						UGroup.all = [];
						UBus.initClass;
						RichBuffer.allBuffers.as(Array).do(_.free);
						AbstractRichBuffer.initClass;
					}
				},
				'reboot servers', {
